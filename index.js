...
function getAllMovieTitles(movies) {
  if (!movies.length) {
    throw "The inputted array is empty.";
  }
...
function getAllMoviesReleasedAtOrBeforeYear(movies, year) {
  ...
  return movies.filter(movie => {return parseInt(movie.released.split(" ")[2]) <= year});
}
...
function getRottenTomatoesScoreByMovie(movies) {
  ...
  return movies.map(movie => {
    ...
    const score = rtRating ? rtRating.value : "N/A";
    return { [movie.title]: score };
  });


  //return movies.map(({title, ratings}) => ({[title]: ratings.find(({source}) => source === "Rotten Tomatoes")?.value || "N/A"}));
}
