...
function checkIfAnyMovieHasRating(movies, rating = "G") {
    ...
    return movies.some(movie => movie.title)
}
...
function findById(movies, id) {
    if (movies.imdbID = ""){
      throw "No imdbID found"
    }
    let imdbID = movies.find(movie => movie.imdbID.toUpperCase() === id.toUpperCase())
    if(!imdbID){
      return null
    }
    return imdbID
...
function filterByGenre(movies, genre) {
  if (!movies.genre){
    throw "No genre found"
  }
return movies.filter(movie => movie.genre.includes(genre).toUpperCase() === genre )
