...
function findById(movies, id) {
  ...
  const result = movies.find(movie => movie.imdbID.includes(id))
  if(!result){
    return null
  }
  return result
...
function getAllMoviesReleasedAtOrBeforeYear(movies, year) {
  ...
  // let released = movies[0].released.split('')
  // let releasedYear = released[2]
  //console.log(movies[0].released.split(" "))
 ...
 */
    function getRottenTomatoesScoreByMovie(movies) {
      ...
      const movieScore = movies.map((movie) => {
        let specScore = movie.ratings.find((rating) => rating.source === "Rotten Tomatoes");
        return { [movie.title]: specScore.value };
      });
