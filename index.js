...
function getAllMovieTitles(movies) {
  ...
  let moviesArray = movies.map((record) => {
    return record.title
  })
  return moviesArray
};
...
function findById(movies, id) {
  if (movies.length === 0) {
    throw "No movies to id"
  };
  let idFound = movies.find((movie)=> {
    if (movie.imdbID === id) {
      return movie
    }
  })
...
function filterByGenre(movies, genre) {
  if (movies.length === 0) {
    throw "Still not available"
  }; 
  let genMovie = movies.filter((movie) => {
...
function checkMinMetascores(movies, metascore) {
  if (movies.length === 0) {
    throw "Another empty box"
  };
  return movies.every((movie) => {
    if (movie.metascore >= metascore) {
      return true
    } else if (movie.metascore <= metascore) {
      return false
    }
  })
};
...
function getRottenTomatoesScoreByMovie(movies) {
  if (movies.length === 0) {
    throw "Netflix and chill tonight"
  };
  return movies.map((movie) => {
    let rottenTomato = {};
//key:movie.title & value:score received from Rotten Tomatoes
    rottenTomato[movie.title] = movie.ratings.find((rating) => rating.source === "Rotten Tomatoes").value
    return rottenTomato//{ Fantasia: "95%" }
  })
};
